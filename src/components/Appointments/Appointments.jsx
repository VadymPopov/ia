...
const handleUpdate = (id, info) => {
  ...
  return (
    <>
     {appointments && appointments.length === 0 && <Text>Oh, how sad, there are no appointments for today. You need to work harder or just relax and chill!</Text>}
    
    <List>
        {appointments && appointments.map((element, idx)=>{
                return <Card key={element._id} data={element} onDelete={handleDelete} onUpdate={handleUpdate}>
                </Card>})
        }
    </List>
    </>
