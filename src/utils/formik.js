...
export const validationSchemaWaiverForm = (isClientUnder18) => {
  ...
  pain: Yup.boolean().oneOf([true], 'Please accept the Pain field').required('Pain is required'),
  infection: Yup.boolean().oneOf([true], 'Please accept the Infection field').required('Infection is required'),
  healing: Yup.boolean().oneOf([true], 'Please accept the Healing field').required('Healing is required'),
  outcome: Yup.boolean().oneOf([true], 'Please accept the Outcome field').required('Outcome is required'),
  refund: Yup.boolean().oneOf([true], 'Please accept the Refund field').required('Refund is required'),
  permanentChange: Yup.boolean().oneOf([true], 'Please accept the Permanent Change field').required('Permanent Change is required'),
  media: Yup.boolean().oneOf([true], 'Please accept the Media field').required('Media is required'),
  age: Yup.boolean().oneOf([true], 'Please accept the Age field').required('Age is required'),
  agreement: Yup.boolean().oneOf([true], 'Please accept the Agreement field').required('Agreement is required'),
  drugs: Yup.boolean().oneOf([true], 'Please accept the Drugs field').required('Drugs is required'),
  desease: Yup.boolean().oneOf([true], 'Please accept the Disease field').required('Disease is required'),
  medication: Yup.boolean().oneOf([true], 'Please accept the Medication field').required('Medication is required'),
  skin: Yup.boolean().oneOf([true], 'Please accept the Skin field').required('Skin is required'),
  recipientOrgan: Yup.boolean().oneOf([true], 'Please accept the Recipient Organ field').required('Recipient Organ is required'),
  ...
  {
    is: ()=> isClientUnder18 === true,
    then: (schema) => schema.min(3).matches(nameRegExp, 'Please enter a valid name').required('Parental/Guardian name is required.'),
    otherwise: (schema) => schema}
  ),
  parentGovernmentId: Yup.string().when([],
  {
    is: ()=> isClientUnder18 === true,
    then: (schema) => schema.matches(governmentId, 'Please enter a valid government-issued ID number').required('Parental/Guardian Government ID is required.'),
    otherwise: (schema) => schema}
  ),
})};
